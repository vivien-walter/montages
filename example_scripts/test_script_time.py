...
# on each image of the montage as well as the time stamp.
...
# 4.1 Set the selection of frames to be saved
images.setSelection(
                    ...
                    maxFrame=9 # (OPT.) Select the maximum number of frames to select for the montage - if set to None
                    )

# 4.2 Set the time scale of the images
images.setTimeScale(
                    scale=5, # (OPT.) Specify the time scale - if not specified, the scale is set to 1
                    unit='ms' # (OPT.) Specify the time unit - if not specified, the unit is set to frame
                    )

# STEP 5: Edit the image to add the desired text

# 5.1 We first add the time stamp at the top of the image
images.setText(
                textType='time', # (OPT.) Type of text to write, can chose between 'file' name and 'time' stamp - Default is 'file'
                textSize = 102, # (OPT.) Size for the text - If not specified, the size is calculated automatically
                textFont = 'Arial.ttf', # (OPT.) Font to use for the text - Default is 'Arial.ttf'
                padding = 20, # (OPT.) Distance in pixel between the text and the edge of the image - Default is 10 px
                position = 'top', # (OPT.) Position of the text, can chose between 'top' and 'bottom' - Default is 'bottom'
                color = 'white' # (OPT.) Color of the text, can chose between 'black' and 'white' - Default is 'white'
                )

# 5.2 We then add the name of the file at the bottom of the image
images.setText(
                ...
                color = 'white', # (OPT.) Color of the text, can chose between 'black' and 'white' - Default is 'white'
                resetText=False # (OPT.) Reset the text if set a second time - Default is True
                )
...
mn.saveMontage(montage, fileName='test_time.tif')

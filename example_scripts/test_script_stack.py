...
# This example script presents how the module can be used to open a stack of image
# (*.tiff here), make a montage of the 25 first images and write the name of the file
# on each image.
...
images.setMontage(
                column=6, # (OPT.) Specify the number of columns - if not specified, it will be automatically calculated
                row=5, # (OPT.) Specify the number of rows - if not specified, it will be automatically calculated
                margin=10, # (OPT.) Specify the size of the margin between frames in the montage - if not specified, there will be no margin
                blackMargin = False # (OPT.) Specify is the margin should be black or not - Defaut is true
                )
